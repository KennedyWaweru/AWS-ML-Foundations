...
class Binomial(Distribution):
    ...
    def calculate_stdev(self):
        ...
        self.stdev = self.n * self.p * (1-self.p)# npq np(1-p)
        return self.stdev 
    ...
    def plot_bar(self):
        ...
        num_success = self.n * self.p 
        num_failures = self.n - num_success
        plt.bar([0,1],[num_failures,num_success])
        plt.xlabel("Outcome")
        plt.ylabel("Frequency")
        plt.title(f"Bar plot of Binomial distribution n {self.n}, p {self.p}")
        plt.show()  
    ...
    def pdf(self, k):
        ...
        return math.comb(self.n, k) * (self.p ** k) * ( (1 - self.p) ** (n-k) )
    ...
    def plot_bar_pdf(self):
        ...
        x_vals = [k for k in range(self.n+1)]
        y_vals = [self.pdf(k) for k in range(self.n+1)]

        plt.bar(x_vals, y_vals)
        plt.title(f"Density plot for binomial distribution n {self.n}, p {self.p}")
